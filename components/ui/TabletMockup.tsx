...
export function TabletMockup({ className }: TabletMockupProps) {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      interval = setInterval(() => {
        setCurrentIndex((prev) => (prev + 1) % DASHBOARD_IMAGES.length)
      }, 2000)
    }
  ...
  return (
    <div className={cn("hidden md:block relative mx-auto w-full max-w-6xl px-4", className)}>
      {/* Tablet Frame */}
      <motion.div
      ...
      >
        ...
        >
          ...
          <AnimatePresence mode="wait">
            <motion.img
              key={DASHBOARD_IMAGES[currentIndex]}
              src={DASHBOARD_IMAGES[currentIndex]}
          ...
          >
            ...
            <span className="text-[10px] font-bold text-white uppercase tracking-widest">Demo</span>
          </motion.div>
